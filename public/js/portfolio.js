(function() {
  ...
  $(document).ready(function() {
    var getFilteredPortfoilio;
    getFilteredPortfoilio = function() {
      var filters;
      filters = {};
      ...
      $.ajax({
        ...
        success: function(data) {
          var dataProvider, graph, portfolioSummary, topHoldingHtml, topInvestments, totalInvestment;
    ...
    $('body').on('click', '.apply-portfolio-filters	', function() {
      var urlParams;
      urlParams = '';
    ...
    $('body').on('click', '.reset-portfolio-filters', function() {
    ...
    $('body').on('change', 'select[name="duration"]', function() {
      if ($(this).val()) {
        ...
        return $('select[name="tax_year"]').val('').attr('disabled', true);
      } else {
        $('input[name="duration_from"]').val('').attr('disabled', false);
        $('input[name="duration_to"]').val('').attr('disabled', false);
        return $('select[name="tax_year"]').val('').attr('disabled', false);
      }
    });
    $('body').on('change', 'select[name="tax_year"]', function() {
      if ($(this).val()) {
        $('input[name="duration_from"]').val('').attr('disabled', true);
        $('input[name="duration_to"]').val('').attr('disabled', true);
    ...
    $('body').on('click', '.download-portfolio-report-xlsx', function() {
      var urlParams;
      urlParams = '';
      if ($('select[name="duration"]').val() !== "") {
        urlParams += 'duration=' + $('select[name="duration"]').val();
      ...
      return window.open("/backoffice/portfolio/export-report?" + urlParams);
    });
    return $('body').on('click', '.download-portfolio-report-pdf', function() {
      var chart, cur_div_id, i, images, imgdata, imgdata2, pending, results, seed_logo_base_64;
      ...
      while (i < AmCharts.charts.length) {
        ...
        chart['export'].capture({
          ...
          this.toJPG({
            ...
            var docDefinition, pdf_name;
            docDefinition = void 0;
            pdf_name = void 0;

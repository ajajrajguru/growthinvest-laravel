...
function downloadSendESignPdf($args)
{
    ...
    switch ($stats_pg) {
        ...
        case 'nomination':$data_investor_nomination = get_investor_nominee_data($investor_id);

            $html = get_html_for_nomination_applicationform_pdf($data_investor_nomination, $stats_pg, '', $additionalArgs);
    ...
    $html = $header_footer_start_html . $html . $header_footer_end_html;
    ...
    if ($args['pdfaction'] == "esign") {
        ...
        if (!file_exists($upload_dir['basedir'] . '/' . $tmp_foldername)) {

            mkdir($upload_dir['basedir'] . '/' . $tmp_foldername);

        }

        $destination_dir = $upload_dir['basedir'] . '/' . $tmp_foldername . '/' . $foldername;
        ...
        $investor_data = get_userdata($investor_id);
        require_once '../inc/adobesign2/vendor/sendpdf_for_signature.php';
        ...
        switch ($stats_pg) {
            ...
            case 'nomination':
                ...
                $callbackurl       = site_url() . '/signed-doc-callback/?type=nominee_application';
        ...
        if ($callbackurl != '' & $output_link != '') {
            ...
            $adobe_sign_cc_mails    = ADOBESIGN_ACCOUNT_CC_EMAILS;
            $adobe_sign_cc_mails_ar = explode(',', $adobe_sign_cc_mails);

            $adobesign_args = array('recipient_email' => $investor_data->user_email,
            ...
            send_pdf_for_signature($adobesign_args);
            global $success;
            $success = true;
            return $success;
        } else {
...
function getHtmlForNominationApplicationformPdf($firmStats, $statsPage, $firmId = 0, $additionalArgs)
{
    ...
    //Start Page 4 SEction 5 Verification

    /* $html.= get_section_header_content_seperator(40).'<table cellpadding="5" cellspacing="0" border="'.$table_border.'"  width="100%"   >
    <tr>
    <td  class="blue_heading_div"  >SECTION 7: CONFIRMATION OF VERIFICATION OF IDENTITY CERTIFICATE</td>
    </tr>
    </table>'.get_section_header_content_seperator();

    $nomineeapplication_verified = false;
    if($nomineeapplication_info!="")
    $nomineeapplication_verified = ($nomineeapplication_info['verified']=="yes")?true:false;

    $verification_checkbox_data2[] = array('label_first'=>false,'label'=>'I confirm','checked'=>$nomineeapplication_verified);

    $html.='<table cellpadding="0" cellspacing="5" border="'.$table_border.'"  width="100%" class="section_holder content_font_size1">

    <tr>
    <td colspan="2">Once all the necessary details have been completed, in order to set up a full, investor account with GrowthInvest, we need to do final verification of your client\'s identity. To do this, we use our GrowthInvest partner onfido.com<br/><br/>'.$nomineeapplication_verified.'
    Please confirm that your client wish for us to proceed with this final verification, and move onto next phase
    </td>
    </tr>
    <tr>
    <td colspan="2">'.get_checkbox_html($verification_checkbox_data2) .'
    </td>
    </tr>
    </table>'/*.get_section_header_content_seperator(700).get_nomination_form_footer()* /; */

    //End Page 4 SEction 5 Verification
    ...
    $html.=transferAssetsSubheaders('SECTION 8: CONFIRMATION OF VERIFICATION OF IDENTITY CERTIFICATE' );
    }
    ...
    <tr style="margin-bottom: 0; padding-bottom: 0;">
    <td  colspan="2" style="width: 100%;">
    Please complete this certificate to confirm your identity. This is required by us to meet anti money laundering requirements.<br>

    <b>Important Information</b><br>A separate certificate must be completed for each client (e.g. joint holders, trustee cases and joint life cases). If the payments for the client
    are being made by a third party, the identity of that person must also be verified and confirmation provided in a separate certificate. This
    certificate cannot be used to verify the identity of any client that falls into one of the following categories:
    <ul class="dashed"  >
    ...
    </ul>This certificate must carry an original signature.<br/><br/> We reserve the right to request a copy of the evidence you use to verify the identity of your client.
    </td>
    ...
    $checkbox_data_iconfirm[] = array('label_first'=>true,'label'=>'I confirm that I have certified myself appropriately on the GrowthInvest website, and that I have a valid certificate to that effect.','checked'=>false);
    $html.=get_checkbox_html($checkbox_data_iconfirm).'
    </td>
    </tr>
    </table>
    </td>
    ...
    $checkbox_data_irequested[] = array('label_first'=>true,'label'=>'I have requested to complete the anti money laundry and know your client certificate online and confirm that I’m not eligible to invest prior to having completed that process.','checked'=>false);
    $html.=get_checkbox_html($checkbox_data_irequested).'
    ...
    $html.= transferAssetsSubheaders('MAILING INSTRUCTIONS' );
    $html.='<table cellpadding="0" cellspacing="10" border="1"   class="w100per round_radius" style="margin-top: 0; margin-bottom: 0; padding-top: 0; padding-bottom: 0;">
    ...
    $html.= transferAssetsSubheaders('SIGNATURE' );
    ...
    <tr style="margin-bottom: 0; padding-bottom: 0;">
    <td colspan="2" style="width: 100%;">
    <p style="margin-bottom: 5px;">I hereby confirm that I have read and agree with all the information provided with this form and that I have read and agree to the Growth<span style="color:#2EB6DD;">Invest</span> Terms and Conditions.</p>
    <p style="margin-bottom: 5px;">I confirm that the bank account details in Section 3 are those of my account and (if applicable) i have given my financial adviserinstruction to use this account for cash payments.</p>
    ...
    <td style="Width: 70%;"><div class="inputcss" style="padding: 3px;">&nbsp;';
